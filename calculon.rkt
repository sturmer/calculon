...
;; - Use x and the real division sign for the respective operations
...
(define (push-equal)
  (define result 0)
  (send display$ set-value (number->string result)))
...
(new button% [parent row4]
     [label "="]
     [min-width 30]
     [callback (λ (button event) ; ASK: do we still need the args if unused?
                 push-equal)])
